...
if (is_dir($image_dir)) {
    ...
    foreach ($files as $file) {
        if (in_array(strtolower(pathinfo($file, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif'])) {
            $image_files[] = $base_url . 'assets/images/slider/' . $file;
        }
    }
}

$jenis_dokumen_counts = [];
// PERBAIKAN: Menghapus typo 'Kep. Sekretaris DPRD' dari daftar ini
$jenis_dokumen_list = ["Peraturan Daerah", "Peraturan DPRD", "Keputusan DPRD", "Keputusan Sekretaris DPRD"];
foreach ($jenis_dokumen_list as $tipe) {
    $sql_count_tipe = "SELECT COUNT(*) as total_tipe FROM peraturan WHERE jenis_dokumen = ? AND is_deleted = 0 AND status = 'Berlaku'";
    $stmt_tipe = $conn->prepare($sql_count_tipe);
    if ($stmt_tipe) {
        $stmt_tipe->bind_param("s", $tipe);
        $stmt_tipe->execute();
        $jenis_dokumen_counts[$tipe] = $stmt_tipe->get_result()->fetch_assoc()['total_tipe'];
        $stmt_tipe->close();
    } else {
        $jenis_dokumen_counts[$tipe] = 0;
    }
}
?>
    ...
    <section id="statistik-dokumen-hukum" class="info-cards-section">
        ...
        <div class="cards-grid">
            <div class="info-card card-animate">
                ...
                <div class="card-count"><?php echo $jenis_dokumen_counts["Peraturan Daerah"]; ?></div>
                <div class="card-label">Jumlah Peraturan</div>
            </div>
            <div class="info-card card-animate">
                <i class="fas fa-gavel card-icon"></i>
                <div class="card-title">Peraturan DPRD</div>
                <div class="card-count"><?php echo $jenis_dokumen_counts["Peraturan DPRD"]; ?></div>
                <div class="card-label">Jumlah Peraturan</div>
            </div>
            <div class="info-card card-animate">
                <i class="fas fa-balance-scale card-icon"></i>
                <div class="card-title">Keputusan DPRD</div>
                <div class="card-count"><?php echo $jenis_dokumen_counts["Keputusan DPRD"]; ?></div>
                <div class="card-label">Jumlah Peraturan</div>
            </div>
            <div class="info-card card-animate">
                <i class="fas fa-scroll card-icon"></i>
                <div class="card-title">Kep. Sekretaris DPRD</div>
                <div class="card-count"><?php echo $jenis_dokumen_counts["Keputusan Sekretaris DPRD"]; ?></div>
                <div class="card-label">Jumlah Peraturan</div>
            </div>
        </div>
    </section>
            ...
            data.forEach((item, index) => {
                ...
                if (isMobile) {
                    row.innerHTML = `
                        ...
                        <td><button class="action-btn" onclick="window.location.href='/peraturan/detail/${item.id}'">Detail</button></td>
                    `;
                } else {
