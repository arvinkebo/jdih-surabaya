...
function initSearch() {
  ...
  function renderTable(data) {
    ...
    // PENTING: Panggil fungsi ini setelah tabel dirender ulang
    attachDetailButtonListeners();
  }
  // Fungsi untuk melampirkan event listener ke tombol detail
  function attachDetailButtonListeners() {
    ...
    detailButtons.forEach((button) => {
      button.addEventListener("click", function () {
        ...
        // Panggil fungsi openModal dari modal.js
        if (typeof openModal === "function") {
          openModal("detail_publik.php?id=" + id);
        } else {
          console.error("openModal function not found. Is modal.js loaded?");
        }
      });
    });
  }
  ...
  async function performSearch() {
    ...
    if (data.success) {
      ...
      if (currentKeyword) {
        searchResultInfo.innerHTML = `Hasil Pencarian Untuk: <strong>"${escapeHtml(
          currentKeyword
        )}"</strong>`;
